...
/**
 * 流程梳理：将chunk向量化，将用户提问向量化，计算二者的余弦匹配度，输出最匹配的结果。
 *
 * 优化点：chunk分批次传入 -> 多线程并发传入chunk -> 错误重试
 */
public class embeddingIO {
    public static void main(String[] args) throws Exception {
        // 1. 初始化 EmbeddingClient（替换成你自己的 API Key）
        String apiKey = IO1.API_KEY;
        Embedding client = new Embedding(apiKey);
